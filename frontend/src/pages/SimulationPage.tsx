...
import { Play, Pause, SkipForward, RefreshCw, TrendingUp, TrendingDown, Activity } from 'lucide-react'
...
export default function SimulationPage() {
  ...
  const [prices, setPrices] = useState<PriceData[]>([
    { symbol: 'BTC', price: 67543.21, change: 2.34, volume: 12.5 },
    { symbol: 'ETH', price: 3456.78, change: -1.23, volume: 8.2 },
    { symbol: 'SOL', price: 145.67, change: 5.67, volume: 3.1 },
  ])
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setPrices(prev => prev.map(p => ({
        ...
        volume: p.volume * (0.8 + Math.random() * 0.4)
      })))
  ...
  }, [isRunning, speed, agents, totalRounds])
  ...
  const graphData = useMemo(() => {
    ...
    // 添加交易标的
    prices.forEach(p => {
      nodes.push({ id: p.symbol.toLowerCase(), name: p.symbol, type: 'symbol', val: 40, color: typeColors[p.symbol], isAgent: false })
    })
    ...
    <div className="h-screen flex flex-col bg-gray-50">
      ...
      <header className="border-b border-gray-200 bg-white px-6 py-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-gray-800">双平台模拟运行</h1>
          <span className="px-3 py-1 bg-blue-100 text-blue-700 text-sm rounded-full">Round {currentRound}/{totalRounds}</span>
        </div>
        <div className="flex items-center gap-3">
          <select value={speed} onChange={e => setSpeed(Number(e.target.value))} className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left: Graph + Price */}
        <div className="w-1/2 flex flex-col p-4 gap-4 border-r border-gray-200">
          {/* 行情面板 */}
              ...
              {prices.map(p => (
                <div key={p.symbol} className="bg-gray-50 rounded-xl p-3">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`flex items-center gap-1 text-sm ${p.change >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                      {p.change >= 0 ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
                      {p.change >= 0 ? '+' : ''}{p.change.toFixed(2)}%
                    </span>
                  </div>
                  <div className="text-xl font-bold text-gray-900">${p.price.toLocaleString(undefined, {maximumFractionDigits: 2})}</div>
                  <div className="text-xs text-gray-500">Vol: {p.volume.toFixed(1)}B</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
            <div className="p-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="font-semibold text-gray-800">智能体关系图谱</h2>
              <button className="text-sm text-gray-400 hover:text-gray-600 flex items-center gap-1"><RefreshCw size={14} /></button>
            </div>
        ...
        <div className="w-1/2 flex flex-col p-4 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div className="grid grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <div className="text-3xl font-bold text-purple-600">{actions.filter(a => a.type === 'post').length}</div>
                <div className="text-xs text-gray-500">发帖数</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-emerald-600">{actions.filter(a => a.type === 'trade').length}</div>
                <div className="text-xs text-gray-500">交易数</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-orange-600">{Math.round((actions.filter(a => (a.sentiment || 0) > 0).length / Math.max(actions.length, 1)) * 100)}%</div>
                <div className="text-xs text-gray-500">看多比例</div>
              </div>
            </div>
          </div>
                  ...
                  {actions.filter(a => a.agentId === selectedAgent.id).slice(0, 10).map(action => (
                    <div key={action.id} className="bg-gray-50 rounded-lg p-2 text-sm">
                      <div className="flex items-center justify-between">
                        <span className={`text-xs px-2 py-0.5 rounded ${action.type === 'trade' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'}`}>
                          {action.type}
                        </span>
                        <span className="text-xs text-gray-400">{action.timestamp.toLocaleTimeString()}</span>
                      </div>
