...
export default function SimulationProcessPage() {
  ...
  const config = useMemo<SimConfig>(() => {
    ...
    return {
      duration_hours: consoleConfig.duration,
      rounds_per_hour: consoleConfig.intervalMinutes,
      total_rounds: Math.floor(consoleConfig.duration * 60 / consoleConfig.intervalMinutes),
  ...
  const narrative = useMemo(() => {
    ...
    const eventContent = consoleConfig?.events?.[0]?.content || ''
    return {
      direction: eventContent || '市场对该事件的反应将分化：一部分交易者认为这是利好信号，另一部分则担忧潜在风险。',
      topics,
            ...
            <div className="h-[calc(100%-60px)] relative bg-gray-50">
              <ForceGraph2D
                ...
                nodeCanvasObject={(node: any, ctx, globalScale) => {
                  ...
                  const nodeSize = Math.sqrt(node.val) * 1.8
